...
### Completed: Milestone 3 — Dataset and representation

**Current status (2026-07-26): Milestone 3 representation acceptance complete.** The first corrected immutable Protocol-3 snapshot contains 267 retained valid-click trials from three independent sessions, split by session into 67 train, 100 validation, and 100 test trials. Its schema-2 preprocessing run converted all 267 trials to 64 equal-time canonical positions plus total movement duration with zero skips and a maximum resampling error of 0.048 px. The earlier 200-trial snapshot is retained only for audit because it omitted the explicitly retained early-stopped session during a now-corrected membership query. Collection protocol 3 samples feasible target centers continuously and uniformly with a 12–36 logical-pixel radius. The current Windows/Qt fullscreen renderer does not reliably dispatch post-render callbacks to the controller, so it activates a target synchronously after making it visible and records its reaction timestamp as render-unconfirmed; reaction time is not used by preprocessing and this presentation timestamping must be revisited before any reaction-time analysis. New sessions persist native virtual-screen bounds for the complete start/target edge-distance condition vector required by Milestone 4. It enforces token-bound timeouts, drains the native ring before finalization, and writes buffered Parquet row groups. The native Raw Input path is a preallocated ring with no per-event heap allocation and rejects unsupported absolute mouse packets. GitHub Actions runs the Python/QML suite and Windows native build.
...
### Completed: Milestone 4 — Baselines (2026-07-26)

The immutable schema-2 representation now loads through a verified snapshot-bound model dataset with the complete 21-value geometry, screen-edge, prior-velocity, and DPI condition vector. A shared typed generation contract decodes 129-value equal-time outputs into 64-point screen trajectories, preserves the exact requested start, constrains endpoints inside the requested target, clips virtual-desktop excursions with an explicit count, and handles zero-distance requests with a positive-duration settling path.

Both required baselines are implemented: seeded standardized nearest-neighbor retrieval with optional compatible-neighbor blending, and a data-efficient PCA plus condition-binned diagonal Gaussian mixture. Both persist independently loadable, provenance-bearing artifacts. Model fitting uses only the training split and runs in the JSONL worker; publication is gated on all held-out hard-correctness checks plus same-seed determinism. Migration 8 ties each model to its immutable snapshot, preprocessing run, resolved configuration, code revision, manifest hashes, and validation report.

Acceptance is verified against the corrected 267-trial dataset: 67 training trials and all 200 validation/test trials. Retrieval and PCA-mixture models both recorded zero hard-correctness failures and zero runtime endpoint corrections. Local preview generation measured 0.35 ms for retrieval and 14.67 ms for PCA mixture on the current CPU. The Generator page can build either baseline in a worker and preview the returned trajectory with timing, path, speed, condition-distance, seed, and OOD metadata. It does not inject input.

### Completed: Milestone 5 — Conditional flow (2026-07-26)

The pinned PyTorch 2.6 environment now provides a small internal conditional flow-matching implementation over standardized 129-value equal-time trajectory targets and the shared condition vector. Training is deterministic on CPU, stays outside the GUI process, records training/validation metrics, writes periodic optimizer/model checkpoints, retains the best validation state, and supports Euler or Heun seeded inference. Experiment records persist dataset and preprocessing IDs, resolved configuration, seed, epoch, best loss, checkpoint, status, and published model; worker cancellation marks its attached experiment cancelled while retaining checkpoints.

The small real-data preset completed 120 epochs against the corrected 267-trial snapshot, improved best validation loss from 2.7294 to 1.9379, published a model only after all 200 held-out trajectories passed hard correctness and same-seed validation, and generated a preview in 1.99 ms. Synthetic tests verify loss improvement, deterministic same-seed output, different-seed variation, both lateral modes, checkpoint creation, and save/load parity. The Experiments page launches small or standard presets and reports live persisted experiment state; the Generator page loads flow models through the same interface as both baselines.

### Completed: Milestone 6 — Validation and registry (2026-07-26)

Validation reports now combine hard trajectory correctness with held-out distribution metrics for duration, path-length ratio, endpoint offset, peak speed, maximum and signed lateral deviation, overshoot, and direction reversals. Each metric records generated and real means plus quantile Wasserstein distance. Held-out nearest-condition percentiles calibrate in-distribution, sparse, and out-of-distribution warnings in the Generator page.

Migration 11 adds a separate candidate/validated/active/deprecated registry lifecycle with a database-enforced single active model. The Models page compares intact reports across retrieval, PCA-mixture, and conditional-flow models. Promotion re-hashes the report, confirms that it passed and belongs to the selected model, checks the registry digest, and writes an audit event transactionally. Tests cover successful promotion and tamper rejection. The real conditional-flow model is now active; both baselines remain validated, and every report remains tied to its exact model and dataset.

### Completed: Milestone 7 — Export and portable runtime (2026-07-26)

The active conditional-flow model exports as a standalone, hash-verified ONNX package with binary and NumPy normalization data plus immutable model/dataset provenance. Python ONNX Runtime and the Qt/Python-independent C++20 runtime implement the same seeded SplitMix64/Box-Muller source, Euler/Heun integration, endpoint constraint, equal-time decoder, desktop clipping, and OOD distance. The public C ABI exposes create, generate, free, destroy, and last-error operations with a small RAII C++ wrapper; `mousegen_cli` loads the same package and returns a 64-point JSON trajectory.

Native setup downloads and caches Microsoft's official ONNX Runtime 1.21.0 Windows x64 SDK under the ignored build tree, rejects any archive whose SHA-256 differs from the pinned `5C07BB2805CD666DDA75FA9BFA60E75F2F90D478B952298DD9D55C00740D81BF`, and copies the runtime DLL beside native binaries. Export package hashes are independently checked by both Python and C++ before loading. Fixed-seed PyTorch, Python ONNX, and C++ screen trajectories agree within `1e-3` pixels and one nanosecond; the live C++ probe measured 26.50 ms model load and 0.43 ms generation. Tampered native packages are rejected. The CLI supports portable export and data-only JSON generation without input injection.

### Completed: Milestone 8 — Playback test harness (2026-07-26)

The local-only playback harness is implemented without operating-system input injection. A dedicated time-critical scheduling thread advances a visible marker on MouseMotionLab's own Generator canvas. Playback starts disarmed, requires separate Arm and Start actions for every run, automatically disarms at completion, aborts on Escape/Abort, application focus loss, or at least 12 physical pixels of mouse movement, and cannot start while collection is active. A process-wide generated-playback marker is set only during playback and is cleared on every terminal path so capture backends can exclude generated activity. Synthetic Qt tests verify unarmed refusal, one-shot auto-disarm, movement abort, emergency-stop cleanup, and marker state.

Desktop acceptance is recorded against the running Windows/QML application. The responsive control panel was inspected at 720x450, 980x620, 1280x800, and maximized 1920x1080 display sizes; the complete Generator controls and preview remained visible and the full interface scaled at each size. Generation uses the actual Qt virtual-desktop union rather than the preview widget as its clipping bounds. The trajectory viewer fits that complete desktop with preserved aspect ratio, then exposes 100-400% zoom and visible horizontal/vertical scrollbars for detailed inspection. A real Generator preview completed once and automatically disarmed. Repeated armed runs were observed aborting after a 35-pixel Windows cursor move, an operating-system Escape key event, and application minimization/focus loss. The visible status and generated-playback marker returned to the disarmed terminal state in every case.
